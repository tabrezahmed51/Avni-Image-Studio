...
import { useComputeProviderStore } from '@/stores/useComputeProviderStore';
  ...
  return (
    ...
    >
      {item.loading ? (
        ...
        <>
          <img src={item.imageUrl} alt={item.prompt} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
          {hovered && (
      ...
      {TAG_GROUPS.map((group) => (
        <div key={group.label} className="flex flex-wrap items-center gap-1.5">
          ...
          {group.tags.map((tag) => {
            ...
            return (
              <button key={tag} onClick={() => toggle(tag)} className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[11px] font-medium border transition-all ${active ? group.activeColor : group.color}`}>
                {active ? <X className="w-2.5 h-2.5" /> : <Plus className="w-2.5 h-2.5" />}{tag}
              </button>
            );
          })}
        </div>
...
function InspireButton({ prompt, onSelect }: { prompt: string; onSelect: (p: string) => void }) {
  ...
  return (
    <div className="relative">
      <button onClick={handleInspire} disabled={loading} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-amber-500/15 hover:bg-amber-500/25 border border-amber-500/20 text-amber-300 text-xs font-medium transition-all disabled:opacity-60">
        <Lightbulb className={`w-3.5 h-3.5 ${loading ? 'animate-pulse' : ''}`} />{loading ? 'Thinking…' : 'Inspire Me'}
      </button>
      {open && (
        <div className="absolute top-full left-0 mt-2 z-30 w-80 sm:w-96 glass-card rounded-xl border border-border/60 shadow-2xl p-3 animate-fadeUp">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">{theme ? `Theme: ${theme}` : 'Prompt Ideas'}</span>
            <button onClick={() => setOpen(false)} className="w-5 h-5 rounded-md hover:bg-secondary flex items-center justify-center"><X className="w-3 h-3 text-muted-foreground" /></button>
          </div>
          {loading ? (
            <div className="flex items-center gap-2 py-4 justify-center"><div className="w-4 h-4 rounded-full studio-gradient animate-pulseGlow" /><span className="text-xs text-muted-foreground">Generating ideas…</span></div>
          ) : (
            <div className="space-y-1.5">
              {suggestions.map((s, i) => <button key={i} onClick={() => { onSelect(s); setOpen(false); }} className="w-full text-left px-3 py-2 rounded-lg bg-secondary/50 hover:bg-primary/10 hover:border-primary/30 border border-transparent text-xs text-foreground/90 leading-relaxed transition-all">{s}</button>)}
            </div>
          )}
        </div>
      )}
...
function EditUploadZone({ onImageSelected }: { onImageSelected: (base64: string, preview: string) => void }) {
  ...
  return (
    <div onDragOver={(e) => { e.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={onDrop} onClick={() => inputRef.current?.click()}
      ...
      <p className="text-xs text-muted-foreground text-center"><span className="text-primary font-medium">Click to upload</span> or drag & drop<br />PNG, JPG, WEBP supported</p>
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="relative overflow-hidden rounded-xl border border-border/50 bg-secondary/30 aspect-square">
        {item.loading ? (
          <div className="w-full h-full flex flex-col items-center justify-center gap-2"><div className="w-6 h-6 rounded-full studio-gradient animate-pulseGlow" /><p className="text-[10px] text-muted-foreground">Creating…</p></div>
        ) : item.imageUrl ? (
      ...
      {item.imageUrl && !item.loading && (
        <div className="flex gap-1">
          <Button onClick={() => { const a = document.createElement('a'); a.href = item.imageUrl!; a.download = 'avni-variation.png'; a.click(); }} variant="outline" size="sm" className="flex-1 text-[10px] py-1.5 h-auto border-border/50">
...
function ComputeToggle() {
  const { mode, setMode, checkLocalConnection, isLocalReachable, isChecking, localUrl } = useComputeProviderStore();
...
function PublishButton({ imageUrl, prompt, style, aspectRatio }: { imageUrl: string; prompt: string; style: string; aspectRatio: string }) {
  const [publishing, setPublishing] = useState(false);
  const [published, setPublished] = useState(false);
  const { user } = useAuth();
  const handlePublish = async () => {
    if (!user) { toast.error('Sign in to publish to gallery'); return; }
    setPublishing(true);
    const { error } = await supabase.from('public_gallery').insert({ user_id: user.id, image_url: imageUrl, prompt, style, aspect_ratio: aspectRatio, is_approved: false });
    if (error) toast.error('Publish failed: ' + error.message);
    else { setPublished(true); toast.success('Submitted for gallery review — admin will approve soon!'); }
    setPublishing(false);
  };
  if (published) return <div className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-xs justify-center"><Globe className="w-3 h-3" />Submitted for Review!</div>;
  return (
    <Button onClick={handlePublish} disabled={publishing} variant="outline" className="w-full border-primary/30 hover:border-primary/60 text-xs py-2 text-primary">
      {publishing ? <><RefreshCw className="w-3.5 h-3.5 mr-1 animate-spin" />Publishing…</> : <><Globe className="w-3.5 h-3.5 mr-1" />Publish to Community Gallery</>}
    </Button>
  );
}
...
function UserMenu() {
  ...
  return (
    <div className="relative">
      <button onClick={() => setOpen(o => !o)} className="flex items-center gap-1.5 pl-1 pr-2.5 py-1 rounded-xl bg-secondary/60 hover:bg-secondary border border-border/40 hover:border-primary/40 transition-all">
        {user.avatar ? <img src={user.avatar} alt="" className="w-6 h-6 rounded-lg object-cover" /> : <div className="w-6 h-6 rounded-lg studio-gradient flex items-center justify-center text-[11px] text-white font-bold">{user.username?.[0]?.toUpperCase()}</div>}
      ...
      {open && (
        <div className="absolute top-full right-0 mt-1.5 z-50 w-44 glass-card rounded-xl border border-border/40 shadow-xl overflow-hidden" onClick={() => setOpen(false)}>
          <div className="px-3 py-2.5 border-b border-border/30"><p className="text-xs font-medium text-foreground truncate">{user.email}</p></div>
          {isAdmin && (
...
export default function HomePage() {
  ...
  const handleTagAppend = useCallback((tag: string) => {
    if (tag.startsWith('__remove__')) {
      ...
      setPrompt(prev => prev.toLowerCase().includes(tag.toLowerCase()) ? prev : prev.trim() ? `${prev.trim()}, ${tag}` : tag);
    }
  }, []);
  ...
  const handleAvniAction = useCallback((action: AvniAction) => {
    switch (action.type) {
      case 'fill_prompt': setPrompt(String(action.payload)); setActiveTab('generate'); document.getElementById('generator')?.scrollIntoView({ behavior: 'smooth' }); break;
      case 'navigate': if (action.payload === 'top') window.scrollTo({ top: 0, behavior: 'smooth' }); else document.getElementById(String(action.payload))?.scrollIntoView({ behavior: 'smooth' }); break;
      case 'set_style': setStyle(String(action.payload)); break;
      case 'set_aspect_ratio': setAspectRatio(String(action.payload)); break;
      case 'open_history': setHistoryOpen(true); break;
      case 'switch_tab': setActiveTab(action.payload === 'edit' ? 'edit' : 'generate'); document.getElementById('generator')?.scrollIntoView({ behavior: 'smooth' }); break;
  ...
  const openGalleryLightbox = useCallback((index: number) => {
    ...
    setLightboxImages(imgs); setLightboxIndex(index); setLightboxOpen(true);
  }, [gallery]);

  const openResultLightbox = useCallback((imageUrl: string, promptText: string) => {
    setLightboxImages([{ imageUrl, prompt: promptText }]); setLightboxIndex(0); setLightboxOpen(true);
  }, []);
  ...
  const handleGenerateVariations = async () => {
    ...
    const initVariations: VariationItem[] = VARIATION_SUFFIXES.map((suffix, i) => ({ id: `var-${i}`, imageUrl: null, loading: true, suffix }));
  ...
  const handleDownloadAll = async () => {
    ...
    try {
      await downloadAllImagesAsZip(readyImages.map((g, i) => ({ url: g.imageUrl, filename: `avni-gallery-${i + 1}.png` })), (packed, total) => setZipProgress({ packed, total }));
  ...
  return (
    <>
      ...
      {/* Avni AI Chat */}
      <AvniAIChat currentPrompt={prompt} onAction={handleAvniAction} onTriggerGenerate={handleGenerate} />
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 glass-card border-b border-border/30 px-4 sm:px-6 py-3">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-5 text-xs text-muted-foreground">
            ...
            <Link to="/gallery" className="hover:text-foreground transition-colors flex items-center gap-1 text-primary/80 text-xs"><Globe className="w-3 h-3" />Community</Link>
          </div>
          <div className="flex items-center gap-2">
            <ComputeToggle />
            <button onClick={() => setSettingsOpen(true)} className="w-8 h-8 rounded-lg bg-secondary/60 hover:bg-secondary border border-border/40 hover:border-primary/40 flex items-center justify-center transition-all" title="AI Integrations">
              <Settings2 className="w-3.5 h-3.5 text-muted-foreground" />
            </button>
            <button onClick={() => setHistoryOpen(true)} className="relative w-8 h-8 rounded-lg bg-secondary/60 hover:bg-secondary border border-border/40 hover:border-primary/40 flex items-center justify-center transition-all">
              <History className="w-3.5 h-3.5 text-muted-foreground" />
              {history.length > 0 && <span className="absolute -top-1 -right-1 w-3.5 h-3.5 studio-gradient rounded-full text-[8px] text-white font-bold flex items-center justify-center">{history.length > 9 ? '9+' : history.length}</span>}
            </button>
            <UserMenu />
            <Button onClick={() => scrollTo('generator')} size="sm" className="studio-gradient text-white border-0 hover:opacity-90 text-xs px-3 py-1.5 h-auto hidden sm:inline-flex">Studio</Button>
          </div>
        </div>
      </nav>
      ...
      <section className="relative pt-24 pb-16 px-4 sm:px-6 text-center overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <Button onClick={() => scrollTo('generator')} size="default" className="studio-gradient text-white border-0 text-sm px-6 py-2.5 rounded-xl hover:opacity-90 glow-violet"><Wand2 className="w-4 h-4 mr-1.5" />Start Creating<ArrowRight className="w-3.5 h-3.5 ml-1.5" /></Button>
            <Button onClick={() => scrollTo('gallery')} variant="outline" size="default" className="text-sm px-6 py-2.5 rounded-xl border-border hover:border-primary/50"><ImageIcon className="w-4 h-4 mr-1.5" />View Gallery</Button>
          </div>
          <button onClick={() => scrollTo('generator')} className="mt-10 flex flex-col items-center gap-1.5 mx-auto text-muted-foreground hover:text-foreground transition-colors animate-float"><span className="text-[10px] uppercase tracking-widest">Explore</span><ChevronDown className="w-4 h-4" /></button>
        </div>
      </section>
      ...
      <section id="features" className="py-16 px-4 sm:px-6">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-10">
            <h2 className="font-display text-2xl sm:text-3xl md:text-4xl font-bold text-foreground mb-3">Everything You Need to <span className="text-studio">Create</span></h2>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="glass-card rounded-2xl p-4 sm:p-6 glow-violet">
            ...
            <div className="flex gap-1 p-1 bg-secondary/50 rounded-xl mb-5">
              <button onClick={() => setActiveTab('generate')} className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-xs font-medium transition-all ${activeTab === 'generate' ? 'studio-gradient text-white shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}><Sparkles className="w-3.5 h-3.5" />Generate</button>
              <button onClick={() => setActiveTab('edit')} className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-xs font-medium transition-all ${activeTab === 'edit' ? 'studio-gradient text-white shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}><Upload className="w-3.5 h-3.5" />Edit Image</button>
            </div>
            ...
            {activeTab === 'generate' && (
              <div className="space-y-4">
                <div>
                  <div className="flex items-center justify-between mb-1.5"><label className="text-xs font-medium text-muted-foreground">Your Prompt</label><InspireButton prompt={prompt} onSelect={setPrompt} /></div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    ...
                    {STYLE_PRESETS.map(s => <button key={s.value} onClick={() => setStyle(s.value)} className={`px-2.5 py-1 rounded-lg text-[11px] font-medium transition-all ${style === s.value ? 'studio-gradient text-white' : 'bg-secondary text-muted-foreground hover:text-foreground'}`}>{s.label}</button>)}
                  </div>
                </div>
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-1.5">
                    {ASPECT_RATIOS.map(ar => (
                      <button key={ar.value} onClick={() => setAspectRatio(ar.value)} className={`py-2 rounded-xl text-[11px] font-medium transition-all flex flex-col items-center gap-0.5 ${aspectRatio === ar.value ? 'studio-gradient text-white' : 'bg-secondary text-muted-foreground hover:text-foreground'}`}>
                        <span className="text-sm">{ar.icon}</span>{ar.label}
                      </button>
                    ))}
                  </div>
                </div>
                <Button onClick={handleGenerate} disabled={generating || !prompt.trim()} size="default" className="w-full studio-gradient text-white border-0 text-sm py-2.5 rounded-xl hover:opacity-90 disabled:opacity-50">
                ...
                {generatedImage && (
                  <div className="space-y-3 animate-fadeUp">
                    <div className="relative overflow-hidden rounded-xl border border-primary/30 cursor-zoom-in" onClick={() => openResultLightbox(generatedImage, prompt)}>
                      <img src={generatedImage} alt="Generated" className="w-full h-auto block" />
                      <div className="absolute top-2 right-2 w-7 h-7 rounded-lg bg-black/50 flex items-center justify-center pointer-events-none"><ImageIcon className="w-3.5 h-3.5 text-white/70" /></div>
                    </div>
                    <div className="grid grid-cols-3 gap-2">
                      <Button onClick={() => { const a = document.createElement('a'); a.href = generatedImage; a.download = 'avni-studio.png'; a.click(); }} variant="outline" className="border-border hover:border-primary/50 text-xs py-2"><Download className="w-3.5 h-3.5 mr-1" />Download</Button>
                      <ShareButton imageUrl={generatedImage} prompt={prompt} />
                      <Button onClick={handleGenerate} variant="outline" className="border-border hover:border-primary/50 text-xs py-2" disabled={generating}><RefreshCw className="w-3.5 h-3.5 mr-1" />Again</Button>
                    </div>
                    <PublishButton imageUrl={generatedImage} prompt={prompt} style={style} aspectRatio={aspectRatio} />

                    {/* Variations */}
                    <div className="pt-1">
                      <div className="flex items-center justify-between mb-3">
                        <div><p className="text-xs font-semibold text-foreground">Generate Variations</p><p className="text-[10px] text-muted-foreground">3 unique takes on your prompt</p></div>
                        <Button onClick={handleGenerateVariations} disabled={generatingVariations} size="sm" className="studio-gradient text-white border-0 text-xs px-3 py-1.5 h-auto hover:opacity-90 disabled:opacity-50">
                          {generatingVariations ? <><RefreshCw className="w-3 h-3 mr-1 animate-spin" />Creating…</> : <><Sparkles className="w-3 h-3 mr-1" />Variations</>}
                        </Button>
                      </div>
                      {variations.length > 0 && <div className="grid grid-cols-3 gap-3 animate-fadeUp">{variations.map(v => <VariationCard key={v.id} item={v} prompt={prompt} />)}</div>}
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-4">
                {editImagePreview ? (
                  <div className="relative rounded-xl overflow-hidden border border-border/50">
                    ...
                    <button onClick={() => { setEditImageBase64(null); setEditImagePreview(null); setEditResult(null); }} className="absolute top-2 right-2 w-6 h-6 rounded-lg bg-black/60 hover:bg-black/80 flex items-center justify-center"><X className="w-3.5 h-3.5 text-white" /></button>
                ...
                <div>
                  ...
                  <textarea value={editPrompt} onChange={(e) => setEditPrompt(e.target.value)} placeholder="Turn this into a watercolor painting, add dramatic storm clouds, change to sunset…" rows={3}
                    className="w-full bg-secondary/50 border border-border rounded-xl px-3 py-2.5 text-foreground placeholder:text-muted-foreground resize-none focus:outline-none focus:ring-2 focus:ring-primary/50 text-xs sm:text-sm leading-relaxed" />
                </div>
                <Button onClick={handleEditGenerate} disabled={editGenerating || !editPrompt.trim() || !editImageBase64} size="default" className="w-full studio-gradient text-white border-0 text-sm py-2.5 rounded-xl hover:opacity-90 disabled:opacity-50">
                ...
                {editResult && editImagePreview && (
                  <div className="space-y-3 animate-fadeUp">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <Button onClick={() => { const a = document.createElement('a'); a.href = editResult; a.download = 'avni-edit.png'; a.click(); }} variant="outline" className="border-border hover:border-primary/50 text-xs py-2"><Download className="w-3.5 h-3.5 mr-1" />Download</Button>
                      <ShareButton imageUrl={editResult} prompt={editPrompt} />
                    </div>
                    <Button variant="outline" size="sm" className="w-full text-xs border-border/50" onClick={() => openResultLightbox(editResult, editPrompt)}><ImageIcon className="w-3 h-3 mr-1.5" />View Fullscreen</Button>
                  </div>
                )}
              </div>
            )}
      ...
      <section id="gallery" className="py-16 px-4 sm:px-6">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-8">
            <h2 className="font-display text-2xl sm:text-3xl md:text-4xl font-bold mb-3">AI-Generated <span className="text-studio">Gallery</span></h2>
            <p className="text-muted-foreground text-sm max-w-lg mx-auto mb-3">Every image was created by our AI — click to expand, hover to download.</p>
            <div className="flex items-center justify-center gap-3">
              <Button onClick={handleDownloadAll} variant="outline" size="sm" className="border-primary/30 hover:border-primary/60 text-xs px-4 py-2 h-auto" disabled={!!zipProgress || gallery.every(g => g.loading)}>
                {zipProgress ? <><Package className="w-3.5 h-3.5 mr-1.5 animate-pulse" />Packing {zipProgress.packed}/{zipProgress.total}…</> : <><Package className="w-3.5 h-3.5 mr-1.5" />Download All as ZIP</>}
              </Button>
              <Link to="/gallery"><Button variant="outline" size="sm" className="border-primary/30 hover:border-primary/60 text-xs px-4 py-2 h-auto text-primary"><Globe className="w-3.5 h-3.5 mr-1.5" />Community Gallery</Button></Link>
            </div>
          </div>
          <div className="columns-1 sm:columns-2 lg:columns-3 gap-3 space-y-3">
            {gallery.map((item, i) => {
              const loadedBefore = gallery.slice(0, i).filter(g => !g.loading && g.imageUrl);
              return <div key={item.id} className="break-inside-avoid"><GalleryCard item={item} onClick={() => openGalleryLightbox(loadedBefore.length)} /></div>;
            })}
            {gallery.length === 0 && <div className="col-span-3 flex items-center justify-center py-20 text-muted-foreground"><div className="text-center space-y-2"><div className="w-10 h-10 studio-gradient rounded-full mx-auto animate-pulseGlow" /><p className="text-sm">Loading gallery…</p></div></div>}
          </div>
        </div>
      </section>
      ...
      <section className="py-16 px-4 sm:px-6">
        <div className="max-w-2xl mx-auto text-center glass-card rounded-2xl p-8 sm:p-10 glow-violet">
          ...
          <h2 className="font-display text-2xl sm:text-3xl md:text-4xl font-bold mb-3">Your <span className="text-studio">Imagination</span> is the Only Limit</h2>
          <p className="text-muted-foreground mb-6 text-sm">Start generating stunning images today.</p>
          <Button onClick={() => scrollTo('generator')} size="default" className="studio-gradient text-white border-0 text-sm px-8 py-2.5 rounded-xl hover:opacity-90"><Wand2 className="w-4 h-4 mr-1.5" />Create Your First Image</Button>
        </div>
      </section>
      ...
      <footer className="border-t border-border/30 py-6 px-4 sm:px-6 pb-20">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-3 text-xs text-muted-foreground">
          <div className="flex items-center gap-2">
            <div className="w-5 h-5 studio-gradient rounded-md flex items-center justify-center"><Sparkles className="w-2.5 h-2.5 text-white" /></div>
