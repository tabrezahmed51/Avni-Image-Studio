...
import {
  ...
  History, Share2, Upload, X, Lightbulb, Plus,
} from 'lucide-react';
import { Button } from '@/components/ui/button';
import { toast } from 'sonner';
import { generateImage, getInspirePrompts, convertImageToBase64 } from '@/lib/api';
        ...
        <>
          ...
          {hovered && (
            <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent flex flex-col justify-end p-3">
              ...
              <button
                onClick={() => { const a = document.createElement('a'); a.href = item.imageUrl; a.download = 'avni-studio.png'; a.click(); }}
                className="mt-1.5 flex items-center gap-1 text-studio-gold text-xs font-medium hover:underline w-fit"
              >
...
function PromptTagBuilder({ prompt, onAppend }: { prompt: string; onAppend: (tag: string) => void }) {
  ...
  const toggle = (tag: string) => {
    ...
    if (next.has(tag)) {
      next.delete(tag);
      // Remove from prompt — replace ", tag" or "tag, " or just "tag"
      onAppend(`__remove__${tag}`);
    } else {
      next.add(tag);
      onAppend(tag);
    }
  ...
  // Sync active tags with current prompt value
  useEffect(() => {
...
function InspireButton({ prompt, onSelect }: { prompt: string; onSelect: (p: string) => void }) {
  ...
  const handleInspire = async () => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : 'Failed to get inspirations';
      toast.error(msg);
      setOpen(false);
    } finally {
      setLoading(false);
  ...
  return (
    <div
      ...
      className={`w-full flex flex-col items-center justify-center gap-2 py-6 rounded-xl border-2 border-dashed cursor-pointer transition-all
        ${dragging ? 'border-primary/70 bg-primary/10' : 'border-border/50 hover:border-primary/40 hover:bg-secondary/30'}`}
    >
...
// ─── Share Button ────────────────────────────────────────────────────
function ShareButton({ imageUrl, prompt }: { imageUrl: string; prompt: string }) {
  const handleShare = async () => {
    const shareData = {
      title: 'Avni Image Studio',
  ...
  return (
    <Button onClick={handleShare} variant="outline" className="flex-1 border-border hover:border-primary/50 text-sm py-2">
...
export default function HomePage() {
  ...
  const handleTagAppend = useCallback((tag: string) => {
    if (tag.startsWith('__remove__')) {
      ...
      setPrompt((prev) => {
        const cleaned = prev.replace(new RegExp(`,?\\s*${t}\\s*,?`, 'gi'), '').replace(/,\s*$/, '').trim();
        return cleaned;
      });
    } else {
      setPrompt((prev) => {
        const lower = prev.toLowerCase();
        if (lower.includes(tag.toLowerCase())) return prev;
        return prev.trim() ? `${prev.trim()}, ${tag}` : tag;
      });
    }
  }, []);

  // Seed gallery on mount
  useEffect(() => {
    if (galleryLoaded) return;
    setGalleryLoaded(true);
    const items: GalleryItem[] = GALLERY_PROMPTS.map((p) => ({
  ...
  return (
    <div className="min-h-screen bg-background mesh-bg overflow-x-hidden">
      ...
      <section className="relative pt-24 pb-16 px-4 sm:px-6 text-center overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto">
          ...
          <h1 className="font-display text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-black leading-[1.05] mb-4">
            <span className="text-studio">Avni</span>
            <br />
            <span className="text-foreground">Image Studio</span>
          </h1>

          <p className="text-base sm:text-lg md:text-xl text-studio-gold font-display italic mb-3">
            "Imagination is next reality"
          </p>

          <p className="text-muted-foreground text-sm sm:text-base max-w-xl mx-auto mb-8 leading-relaxed">
            Transform your words into breathtaking visuals. Multi-modal AI at your fingertips —
            generate, edit, and create stunning images in seconds.
          </p>

          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <Button onClick={() => scrollTo('generator')} size="default" className="studio-gradient text-white border-0 text-sm px-6 py-2.5 rounded-xl hover:opacity-90 glow-violet transition-all">
        ...
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-10">
            ...
            <p className="text-muted-foreground text-sm max-w-lg mx-auto">
              A complete creative suite powered by the world's most advanced AI models.
            </p>
          </div>
        ...
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-8">
            <h2 className="font-display text-2xl sm:text-3xl md:text-4xl font-bold mb-2">
              <span className="text-studio">AI Studio</span>
            </h2>
          ...
          <div className="glass-card rounded-2xl p-4 sm:p-6 glow-violet">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    <button onClick={() => setStyle('')} className={`px-2.5 py-1 rounded-lg text-[11px] font-medium transition-all ${style === '' ? 'studio-gradient text-white' : 'bg-secondary text-muted-foreground hover:text-foreground'}`}>
                      None
                    </button>
                    {STYLE_PRESETS.map((s) => (
                      <button key={s.value} onClick={() => setStyle(s.value)} className={`px-2.5 py-1 rounded-lg text-[11px] font-medium transition-all ${style === s.value ? 'studio-gradient text-white' : 'bg-secondary text-muted-foreground hover:text-foreground'}`}>
                        {s.label}
                      </button>
                    ))}
                  </div>
                ...
                <Button
                  ...
                  size="default"
                  className="w-full studio-gradient text-white border-0 text-sm py-2.5 rounded-xl hover:opacity-90 disabled:opacity-50 transition-all"
                >
                  {generating ? (
                    <><RefreshCw className="w-4 h-4 mr-1.5 animate-spin" />Generating…</>
                  ) : (
                    <><Sparkles className="w-4 h-4 mr-1.5" />Generate Image</>
                  )}
                </Button>
                  ...
                  <div className="space-y-3 animate-fadeUp">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      <Button
                        ...
                        variant="outline"
                        className="border-border hover:border-primary/50 text-xs py-2"
                      >
            ...
            {activeTab === 'edit' && (
              <div className="space-y-4">
                ...
                >
                  {editGenerating ? (
                    <><RefreshCw className="w-4 h-4 mr-1.5 animate-spin" />Editing…</>
                  ) : (
                    <><Wand2 className="w-4 h-4 mr-1.5" />Edit Image</>
                  )}
                </Button>
                ...
                {editResult && (
                  <div className="space-y-3 animate-fadeUp">
                    <div className="relative overflow-hidden rounded-xl border border-primary/30">
                      <img src={editResult} alt="Edited" className="w-full h-auto block" />
                      <div className="absolute top-2 left-2 text-[10px] bg-black/60 text-white px-2 py-0.5 rounded-md">Edited result</div>
                    </div>
      ...
      <section id="gallery" className="py-16 px-4 sm:px-6">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-8">
            ...
            <p className="text-muted-foreground text-sm max-w-lg mx-auto">
              Every image was created by our AI — hover to see the prompt, click to download.
            </p>
          </div>
      ...
      {/* ── FOOTER ── */}
      <footer className="border-t border-border/30 py-6 px-4 sm:px-6">
